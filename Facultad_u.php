...
<?php
  if($_SERVER["REQUEST_METHOD"] =="POST"){ 
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://crai.informaticapp.com/facultades/'.$_POST['idfacultad'],
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://crai.informaticapp.com/facultades/'.$_GET['idfacultad'],
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
...
<body class="hold-transition sidebar-collapse sidebar-mini layout-fixed layout-navbar-fixed">
  ...
  <h3 class="text-center">Administrar Facultad</h3>
  <div class="container">
    <form method="post" class="col-xl-8 offset-2">
      <input type="hidden" name="idfacultad" id="idfacultad" value="<?= $data_FA["Detalles"]['idfacultad'] ?>">
      <div class="form-group">
        <label for="nombre">Facultad:</label>
        <input type="text" name="nombre" class="form-control" id="nombre" value="<?= $data_FA["Detalles"]['nombre'] ?>"/>
      </div>
      <div class="form-group">
        <label for="Siglas">Siglas:</label>
        <input type="text" name="siglas" class="form-control" id="siglas" value="<?= $data_FA["Detalles"]['siglas'] ?>"/>
      </div>
